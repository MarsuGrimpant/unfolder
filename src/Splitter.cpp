...
namespace masc {
	...
	vector<float> SteepestEdgeSplitter::assignWeights(model* m,
...

		{
			...
			cout << endl << endl << "TOP VERTEX ID  " << top_vertex_id << endl << endl;
		}
	...
	vector<float> MySplitter01::assignWeights(model *m, const Config& config)
	{
		...
		for (auto i = 0; i < m->v_size; ++i) {
			...
			float max_prod = -FLT_MAX;
			int greatest_increase_eid = INT_MAX;
			...
			for (auto eid : v.m_e) {
				...
				if (prod > max_prod) {
					max_prod = prod;
					greatest_increase_eid = eid;
				}

				if (greatest_increase_eid == INT_MAX) {
					cout << "greatest increase edge not found for vertex " << i << endl;
				}
				else {
					selected_edges.insert(greatest_increase_eid);
				}
			}
		}
	...
	vector<float> MySplitter02::assignWeights(model *m, const Config& config)
	{
		...
		// find the barycenter of the model P

		Point3d barycenter;
		double * values = barycenter.get();
		for (auto i = 0; i < m->v_size; ++i) {
			barycenter = barycenter + Vector3d(m->vertices[i].p.get());
			double * valuess = m->vertices[i].p.get();
		}
		barycenter.set(values[0] / m->v_size, values[1] / m->v_size, values[2] / m->v_size);

		// Find the rightmost ascending edge 
		...
		for (auto i = 0; i < m->v_size; ++i) {
			...
			float max_dist_avg = -FLT_MAX; // Average of the rightmost ascending edge from v to barycenter and from v to w w.r.t c
			int rightmost_ascending_eid = INT_MAX;
			...
			for (auto eid : v.m_e) {
				...
				const float incident_e_factor = (c * vw) / vw.norm();

				// If > 0, determine the maximum average
				if (incident_e_factor > 0) {
					const Vector3d bv = v.p - barycenter;
					const float bv_factor = (c*bv) / bv.norm();

					if ((bv_factor + incident_e_factor) / 2 > max_dist_avg) {
						max_dist_avg = (bv_factor + incident_e_factor) / 2;
						rightmost_ascending_eid = eid;
					}
				}

			}
			if (rightmost_ascending_eid == INT_MAX) {
				cerr << "rightmost ascending edge not found for vertex " << i << endl;
			}
			else {
				selected_edges.insert(rightmost_ascending_eid);
			}
		}
		// assign weight
		for (auto i = 0; i < m->e_size; ++i) {
